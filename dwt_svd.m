...
if (cover_fname ~= 0)
    ...
    cover_image = imresize(cover_image, [512 512], 'bilinear'); %resize image = noi suy Bilinear 2 by 2
else
    return;
end
...
if (watermark_fname ~= 0)
	...
	%im2bw() convert grayscale img to binary image based on threshold (default 0.5)
...
if (gaussian_plot == false)
    ...
    wrongWatermarkDiff = zeros(1, length(watermark_logo));  %length(watermark_logo) return number of col
    ...
    for key = 1:numOfKeys
        ...
        % the interval [-1, 1]
        original_signature(find(original_signature == 0)) = -1; %tìm ki?m v? trí nào = 0, thay = -1, t?o ra matrix -1:1
        
        % Measure the degree of distortion of the original and the watermarked
        % image using PSNR (dB)
        psnr_values(key) = psnr(cover_image, watermarked_image);
        psnr2dB_values(key) = pow2db(psnr_values(key)); % chuy?n power sang db, ydb=10log10(y)
